...
- 在Ada中，`'Size`是一个属性，用于获取对象或类型的大小（以位为单位）。这个属性在处理底层编程或者需要了解对象或类型大小的场景中很有用。
...
- 对于类属程序包和子程序，都是需要重新给模板函数和模板类重新起一个名字的，最终我们用的时候是用的重新起的这个名字来调用函数或者使用程序包，这个跟c++中的模板是有区别的，c++中的还是用的原来的名字。
...
- 经过验证只有结构体不能直接赋值，需要一个一个得赋值，数组一般可以直接赋值
  - 数组的赋值操作不涉及类型的完全一致性，只要数组元素的类型、大小和结构一致即可。
  - 目前不知道什么原因，有得结构体里面得数组可以直接赋值，但是结构体里面得结构体里面得数组不能直接赋值。用得时候再说吧，看不能赋值得时候挨个赋值吧。
