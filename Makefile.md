...
- 我们先来看一下第一种情况的使用。如果需要书写这样一个规则，规则所定义的命令不是去创建文件，而是通过 make 命令明确指定它来执行一些特定的命令。实例：
  ...
  - 这里要说明一点的是， `clean` 不是一个文件，它只不过是一个动作名字，有点像c语言中的label一样，其冒号后什么也没有，那么，make就不会自动去找它的依赖性，也就不会自动执行其后所定义的命令。要执行其后的命令，就要在make命令后明显得指出这个label的名字。这样的方法非常有用，我们可以在一个makefile中定义不用的编译或是和编译无关的命令，比如程序的打包，程序的备份，等等。
    ...
    - 上面还没有说伪目标，只是说一个目标没有依赖，可能是普通目标没有依赖，也可能是伪目标没有依赖，如果两种都不是第一个目标，且伪目标不是其他目标的依赖时，这种没有依赖的目标都不会执行，这就是上面说的不会去找它的依赖性。

      - 一般情况下不会将不是伪目标的且没有依赖的目标(即普通的没有依赖的目标)作为其他目标的依赖，一般只会将伪目标(且没有依赖)作为其他目标的依赖
      - 通过验证如果一个普通的目标没有依赖时作为其他目标的依赖，也是会执行后面的命令，只是可能会出现如果目录下有一个相同名字的文件时就不会执行了
      - 嵌套执行的时候目录声明成伪目标也是这个意思，如果不声明为伪目标且没有依赖时，在目录下没有相同名字的文件时也是会执行的，声明为伪目标的好处就在于不管有没有相同名字的文件都是会执行的。

    - 如果不用伪目标，而且clean不是第一个目标，我们在执行clean的时候也需要make clean。这样写必须没有依赖，正如上面所说的，其冒号后什么也没有，那么，make就不会自动去找它的依赖性，也就不会自动执行其后所定义的命令。要执行其后的命令，就要在make命令后明显得指出这个label的名字，上面说的都是基于伪目标不是第一个目标，如果伪目标是第一个目标，其后的命令也是会执行的
...
- 伪目标的另一种使用的场合是在 make 的并行和递归执行的过程中，此情况下一般会存在一个变量，定义为所有需要 make 的子目录。对多个目录进行 make 的实现，可以在一个规则的命令行中使用 shell 循环来完成。如下：
  ...
  - 有了伪目标之后，我们可以用它来克服以上方式所存在的两个问题，代码展示如下：
    ...
    -C命令跟的是目录，意思是跳进去那个目录执行make，所以SUBDIRS这个伪目标变量集合写的都是需要执行make命令的那个文件目录
    ```
    
    - 上面的实例中有一个没有命令行的规则“foo:baz”，这个规则是用来规定三个子目录的编译顺序。因为在规则中 "baz" 的子目录被当作成了 "foo" 的依赖文件，所以 "baz" 要比 "foo" 子目录更先执行，最后执行 "bar" 子目录的编译。

- 一般情况下，一个伪目标不作为另外一个目标的依赖。这是因为当一个目标文件的依赖包含伪目标时，每一次在执行这个规则伪目标所定义的命令都会被执行（因为它作为规则的依赖，重建规则目标时需要首先重建规则的所有依赖文件）。当一个伪目标没有任何（此目标是一个可被创建或者是已存在的文件）依赖时，我们只能通过 make 的命令来明确的指定它的终极目标，执行它所在规则所定义的命令。例如 make clean。
...
- 如何来使用呢？举例说明如下：
  ...
  - 上述的规则也可以换成另外一种写法：
    ...
    ```
    subsystem    
    	$(MAKE) -C subdir   #这个subdir实际上是目标名称，所以一般用$@来取目标，然后每个目标都根据目标跳进去执行make命令，所以目标一般设置为伪目						 标，并且伪目标名称是实际要执行make的目录名，参看下面实例
    ```
...
- Makefile 中还有两个变量不管是不是使用关键字 "export" 声明，它们总会传递到下层的 Makefile 中。这两个变量分别是 SHELL 和 MAKEFLAGS，特别是 MAKEFLAGS 变量，包含了 make 的参数信息。如果执行总控 Makefile 时，make 命令带有参数或者在上层的 Makefile 中定义了这个变量，那么 MAKEFLAGS 变量的值将会是 make 命令传递的参数，并且会传递到下层的 Makefile 中，这是一个系统级别的环境变量。
  ...
  - 上面的意思是说MAKEFLAGS变量都会传递下去，如果在Makefile文件中没有定义的话，MAKEFLAGS变量的值就是在命令行执行make时后面带的参数

  - 如果在Makefile文件中定义了MAKEFLAGS变量，此时就会将这个变量传递下去，就是从一个Makefile中跳到另一个Makefile中时，这种就会将上一个Makefile中的MAKEFLAGS变量带到跳进去的Makefile中，且会一直带下去，例如a -> b -> c，如果a里面定义了MAKEFLAGS变量，c里面也会用这个变量的。如果a有两条线，a -> b -> c , a -> d -> e，相当于a中会跳转两个Makefile   b和d，且其各跳一个，如果b中定义了MAKEFLAGS变量，c中会有，但是d和e是没有的，这个就是一个链路从前往后的过程，如果a定义了，则所有的都会有。
...
- 我们可以看到最外层有一个 Makefile 文件，这就是我们的 "总控Makefile" 文件，我们使用这个 Makefile 调用项目中各个子目录的 Makefile 文件的运行。假设只有我们的 lib 目录下和 app 目录下的各个子目录含有 Makefile 文件。那我们总控的 Makefile 的文件可以这样来写：
  ...
  - 为什么player和libraries要设置为伪目标，因为伪目标的特性是只要执行到伪目标，不会去找寻依赖(看是否文件修改过)，肯定是会执行后面的命令的。伪目标也可能有依赖，例如上面的all，伪目标的特性就是执行到伪目标就会执行后面的命令，如果有依赖，也不看依赖是否是最新的，都是会执行后面的命令，如果有依赖，不管依赖是不是最新的，也都会根据依赖执行后面的命令，总的来说，伪目标是第一个目标或者伪目标是其他目标的依赖时总是会执行后面的命令。
