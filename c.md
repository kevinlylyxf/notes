...
- C语言推出了一种新的类型，叫做 **wchar_t**。w 是 wide 的首字母，t 是 type 的首字符，wchar_t 的意思就是宽字符类型。wchar_t 的长度由编译器决定。微软编译器下长度为2，在GCC、LLVM/Clang下长度为4.wchar_t 其实是用 typedef 关键字定义的一个别名。wchar_t 类型位于 <wchar.h> 头文件中
  ...
  - 目前就认为wchar_t类型是编译器实现的，类似于char int这种，是内部实现的，不是typedef实现的
...
- c语言字符串结束标志\0的含义如下：
  ...
  - 所以在定义字符串数组的时候，在没有赋值的位置直接初始化为0，对于字符串来说是\0，反斜杠的意思是以八进制的形式将ASCII值写进去，第一个值是0，所以书写起来就是\0

  - 为什么我们总说\0而不是说0，因为0我们一般都认为是数字0，并不是ASCII中的第一个编码0，所以我们用\0来代表第一个编码。
  ...
  - 在定义字符数组时一般定义如下
    ...
    - 数组里的每个元素都是数值 0，也就是字符 `'\0'`（NUL）。

- 用转义字符给char赋值时，需要加单引号吗
...
- 总结表格如下
  ...
  | **`double`** | **`unsigned short`**     | 转换为 **`long`**，然后将 **`long`** 转换为 **`unsigned short`** |
...
- 输入设备：scanf()、getchar()、gets() 就是从输入设备（键盘）上读取内容。对于输入设备，没有缓冲区将导致非常奇怪的行为，比如，我们本来想输入一个整数 947，没有缓冲区的话，输入 9 就立即读取了，根本没有机会输入 47，所以，没有输入缓冲区是不能接受的。Windows、Linux、Mac OS 在实现时都给输入设备带上了行缓冲，所以 scanf()、getchar()、gets() 在每个平台下的表现都一致。但是在某些特殊情况下，我们又希望程序能够立即响应用户按键，例如在游戏中，用户按下方向键人物要立即转向，而且越快越好，这肯定就不能带有缓冲区了。Windows 下特有的 getche() 和 getch() 就是为这种特殊需求而设计的，它们都不带缓冲区。
  ...
  - 在不同的操作系统和环境中，实现 `getch` 函数的方式有所不同。在 Windows 系统中，可以直接使用 `conio.h` 中的 `getch` 函数。在 Linux 系统中，需要使用 `ncurses` 库或通过 `termios` 进行手动实现。
...
- 0、’0’、’\0’、”0”、NULL
  ...
  - ’\0’：使用转义字符表示的NULL，相当于直接使用的0。最常见的用法是隐式地表示字符数组结束。

  - ”0”：不是单个的字符，而是一个字符串。实际上它是两个字符，即’0’和’\0’。使用数组来存储时，该数组的长度是2。

  - NULL：同0

- 综上所述，对char类型数组赋值，0也是\0，并不是数字0，对char类型来说，赋值相当于将某一个ASCII赋值进去，0代表ASCII值为0的字符，即NULL，而如果赋值某一个字符例如A，需要写单引号，例如`char * str[10] = {'A'}`，对于char类型来说，直接用整数，对应的就是ASCII值，如果要将某一个符号写进去要用单引号。所以memset可以用0来初始化字符串数组，所有的字节都变成了NULL，在内存中相当于00000000

- 对于int、float这种数值类型的数组赋值来说，默认的是0、0.0，这些在内存中所有的表示也都是二进制的，对应二进制就是ASCII值为0的即NULL，对于int占用4个字节，对应的就是4个NULL，即32为全为0，所以int、float等数值类型也可以用memset(0)初始化。memset初始化对应的是每一个字节，用0初始化int即每个字节都是0，即00000000，最后4个字节在一起表现出来的也是0

- 总的来说，就是要区分数字0和字符'0'的区别，对于memset和默认初始化这些，都是用的数字0，即ASCII对应的值写进去。字符'0'只有在拼写字符串的时候才会用，字符数组也不会用字符'0'来初始化。

  - 不带单引号的0相当于二进制的00000000，然后将这个二进制写进去。
  - 对于我们来说，所有的数字，都是要在内存中的，都是要将数字对应的二进制写进内存中。

- 对于每一个类型来说，最后都是存储的二进制，然后按照不同的类型表现出来，对于字符'0'，其在内存中就是ASCII值为48即00110000，最后看按什么类型表现，如果用int类型表现，就是48，如果打印的时候按照%c表现，就是字符'0'

- 所以对于上面的默认初始化和memset的初始化，都是将0的ASCII值写进去即00000000，对于char来说就是NULL，对于int类型来说4个字节组起来表现也是0
...
- 对于一些比较复杂的结构体数组用memset初始化。即定义完成之后不赋值，直接用memset全部初始化为\0
  - 如果只初始化了某些结构体数组元素，另一些没初始化的默认按照0初始化，当结构体数组只初始化了某些元素，没有初始化的按照默认初始化来赋值，类似于上面
    - 对于short、int、long，就是整数 0；
    - 对于char，就是字符 '\0'；
...
- 字符串赋值的问题
  ...
  - 上面这两种赋值方法是正确的，打印的时候会有空格，什么情况下不能有空格，是在scanf输入的时候不能有空格。因为空格会被当做下一个的分割。
...
- strncmp和数组元素的使用
  ...
  ```
  if(strncmp(".csnp", &filename[strlen(filename)-5], 5) == 0)
    {
      printf("file type is csnp\n");
      analysisFile(filename, CSNP);
    }
    
  int strncmp(const char *s1, const char *s2, size_t n);
  ```

- filename是一个字符数组，存储文件名，我们要判断后缀是否为.csnp，strncmp函数前两个参数都是指针，第二个参数我们找到倒数第五个元素，然后取地址，相当于将指针指向了倒数第五个元素，然后strncmp函数在使用时就直接顺着指针往后移动，第三个参数表示比较五个，所以会移动五个元素。

- 此例提供了一种数组元素和指针的关系，以及指针往后移动的关系

- 所以在函数形参为指针的时候，以及指针需要往后移动的时候，我们就可以使用这种方法。

- 指针记录的只是一个首地址，例如strncmp是需要指针往后移动的，我们将首地址传进去就可以。至于往后移动要符合指针的移动规律。指针+1表示移动一个指针对应元素的字节数。上面strncmp是char *，所以是往后移动一个字节，一个一个比较。

- 这是函数内部写好的，我们将首地址传进去，他在内部往后移动，从首地址开始移动比较几个字符。我们只需要考虑首地址传进去就可以了。
...
- 在程序占用的整个内存中，有一块内存区域叫做栈（Stack），它是专门用来给函数分配内存的，每次调用函数，都会将相关数据压入栈中，包括局部变量、局部数组、形参、寄存器、冗余数据等。栈是针对线程来说的，每个线程都拥有一个栈，如果一个程序包含了多个线程，那么它就拥有多个栈。目前我们编写的程序都是单线程的，所以不必考虑多线程的情况。每次调用函数都会在栈上分配内存，函数调用结束后再释放这一部分内存，内存的分配和释放都是需要时间的。
  ...
  char *reverse(char *str) {
      ...
      if (len > 1) {
          ...
          reverse(str + 1);  //递归调用
          str[len - 1] = ctemp;
      } //每一次函数调用都是一个栈帧，所以临时变量ctemp在每个栈帧上不会被覆盖。
  ```

- 在现代计算机体系结构中，每次函数调用都会在程序的调用栈（call stack）上创建一个新的栈帧（stack frame），而不是一个新的独立的栈。调用栈用于管理函数调用和返回的过程，以及维护每个函数的局部变量和状态。

  - **调用栈（Call Stack）：**
    - 调用栈是一个后进先出（LIFO）数据结构，用于管理函数调用过程。
    - 在程序执行过程中，每当一个函数被调用时，都会在调用栈上分配一个新的栈帧。
    - 当函数返回时，其栈帧会从调用栈中弹出。
  - **栈帧（Stack Frame）：**
    - 每个栈帧包含了函数的局部变量、参数、返回地址（即函数返回后程序继续执行的地址）以及保存的寄存器值等。
    - 栈帧会在函数调用时创建，并在函数返回时销毁。

- 所以在一个线程中栈的大小为8M，这里面包含局部变量、局部数组、形参、寄存器、冗余数据，所有的函数调用都占用这8M的空间。所以递归很容易栈溢出。是所有的函数调用里面的局部变量这些都是在一个栈空间里面，只是有不同的栈帧，但是用的空间是同一份8M

- 栈大小

  - 默认情况下，Linux 的每个线程的栈大小通常是 8 MB。这个值可以通过 `ulimit` 命令查看和修改。

  - 使用 `ulimit -s` 命令可以查看当前的栈大小限制，这个命令将返回栈大小限制（以 KB 为单位）。

  - 使用 `ulimit -s <size>` 命令可以修改栈大小限制，例如：
...
- 影响栈大小的因素
  ...
  1. **递归深度**: 深度递归会消耗大量的栈空间。如果递归深度很大，可能会导致栈溢出。
  2. **局部变量**: 每个函数调用的局部变量会占用栈空间。大量的局部变量会增加栈的使用量。
  3. **线程数量**: 多线程应用程序每个线程都有自己的栈，因此线程数量增加会消耗更多的总栈空间。

- 每个进程的栈大小取决于操作系统的默认设置，可以通过系统配置和编程接口进行调整。了解和管理栈大小对于编写高效、稳定的程序非常重要，尤其是在处理递归或多线程编程时。

##### void修饰函数

- 在C语言中，`void`可以用来修饰函数的返回类型或函数的参数列表。下面是两个常见的用法：

  - **修饰函数的返回类型**： 当一个函数不需要返回任何值时，可以使用`void`作为返回类型。例如：
    ...
    - 在这个例子中，`printMessage`函数执行打印操作，但不返回任何值。

  - **修饰函数的参数列表**： 当一个函数不需要接受任何参数时，可以使用`void`来表示空的参数列表。例如：
    ...
    - 这个例子中的`printMessage`函数同样不接受任何参数。
...
- 在头文件中定义函数和全局变量，这种认知是原则性的错误！不管是标准头文件，还是自定义头文件，都只能包含变量和函数的声明，不能包含定义，否则在多次引入时会引起重复定义错误。
  ...
  - 头文件中不能包含定义，所以在头文件中定义全局变量是错误的，所以全局变量只能定义在源文件中，如果我们需要在include头文件时引入变量，就需要在相应头文件中声明这个变量，然后每个引入了此头文件的就可以用此全局变量了。
...
- c语言全局变量不能在函数外赋值，只能在函数外初始化

  - 在C语言中，全局变量的声明和初始化必须在所有函数外部进行。这是因为全局变量的生命周期和作用域的特殊性质所决定的。具体来说：

    - **全局变量的生命周期**：全局变量的生命周期是整个程序的运行期间。在程序开始运行时，所有全局变量的存储空间就已经分配好，并在程序结束时才释放。因此，全局变量的初始化必须在程序运行前完成。
    - **全局变量的作用域**：全局变量的作用域是从声明开始到文件结束。这意味着全局变量在整个文件范围内都可见，因此需要在编译时就能确定其初始值。

  - 初始化和赋值的区别

    - **初始化**：初始化是在声明变量时为其指定一个初始值。对于全局变量，这个初始化必须在声明时完成，且必须在函数外进行。例如：

      ```c
      int globalVar = 10;  // 全局变量的声明和初始化
      ```

      - 在编译阶段，编译器会为`globalVar`分配内存并将其初始值设置为10。

    - **赋值**：赋值是为已经存在的变量分配一个新值，例如：

      ```c
      void func() {
          globalVar = 20;  // 在函数内部对全局变量赋值
      }
      ```

  - 为什么不能在函数外给全局变量赋值

    - 在C语言中，函数外部的代码主要用于声明和定义，而不是执行语句。赋值操作属于执行语句，需要在程序运行时执行，因此不能在函数外部进行。这是C语言语法的基本规则：

      - 在函数外部只能有声明、定义、预处理指令和一些特定的语句（如`typedef`、`enum`、`struct`等）。

      - 赋值操作需要在运行时执行，所以必须放在函数内部。

        ```c
        #include <stdio.h>
        
        int globalVar;  // 在函数外部声明
        
        globalVar = 10;  // 错误：在函数外部赋值（这是不允许的）
        ...
        - `globalVar = 10;`试图在函数外部进行赋值，这是不合法的C语法。编译器会报错，因为它期望在函数外部只看到声明和定义，而不是赋值语句。

  - 所有的全局变量都不能在函数外赋值，只能在函数外定义和初始化。
...
- 递归函数是在一层栈上分配空间，并不是每一次调用都新开一个栈，所有的调用都是在一个栈上。
...
- C语言不支持像C++那样的显式函数重载（Function Overloading）特性。在C语言中，函数名必须是唯一的，不能有多个同名函数但参数列表不同的情况。因此，无法像在C++中那样定义多个同名函数来处理不同类型或数量的参数。
- 如果在C语言中需要实现类似的功能，可以使用不同的函数名来区分不同的操作

##### C不支持默认形参

- 在标准的C语言中，并没有像C++或者其他一些编程语言中的默认参数（默认形参）的概念。在C语言中，函数的参数必须在调用时显式地传递值，没有办法为参数指定默认值。
- 在调用函数时，必须提供每个参数的值，否则会导致编译错误。
- C语言函数调用时，实参的顺序和类型必须与函数声明的形参一致。
...
- 字符串就是地址，地址就是字符串，在%s输出字符串时，可以只写上字符串，也可以只写字符串地址，不用&
  ...
  - 在 C 和 C++ 语言里，字符串字面量是一种特殊的常量。当程序被编译时，编译器会把字符串字面量存放在内存的常量区，这个区域的内容在程序运行期间是只读的，无法被修改。这一特性不会因为是否有变量接收字符串字面量而改变。
    ...
    - **编译阶段**：编译器会把 `"Hello, World!"` 存放在常量区，并为其分配内存空间。
    - **运行阶段**：当执行 `printMessage("Hello, World!")` 时，实际上是把 `"Hello, World!"` 在常量区的首地址传递给 `printMessage` 函数的参数 `msg`。函数通过这个地址来访问和使用存储在常量区的字符串内容。

  - 在 C 语言中，**双引号括起来的字符串（字符串字面量）作为函数参数传递时，会自动退化为指针**，这是由 C 语言的设计哲学、底层内存管理机制和效率优化共同决定的。以下是详细解释：

    - 字符串字面量的本质
      - 字符串字面量是 `const char[]` 类型的数组，当你写 `"hello"` 时，编译器会：
        - 在程序的**常量区（`.rodata` 段）**分配内存，存储字符序列 `{'h', 'e', 'l', 'l', 'o', '\0'}`。
        - 将其视为一个 `const char[6]` 类型的**匿名数组**（无名数组）

    - 字符串字面量的“值”是其首字符的地址
      - C 语言规定，数组表达式在大多数情况下会退化为指向其首元素的指针（除非用于 `sizeof` 或 `&` 操作）。
      - 因此，`"hello"` 作为表达式时，它的“值”是 `&"hello"[0]`（即 `'h'` 的地址），类型是 `const char*`。
...
- `*`在不同的场景下有不同的作用：`*`可以用在指针变量的定义中，表明这是一个指针变量，以和普通变量区分开；使用指针变量时在前面加`*`表示获取指针指向的数据，或者说表示的是指针指向的数据本身。
  ...
  - `*p`表示获取指针指向的数据，和普通变量i一样，i也是获取数据，所以`*p`是可以当左值的。
...
- 一维数组作为函数参数的时候，编译器会把数组名当作指向该数组第一个元素的指针
...
- 定义指向数组的指针
  ...
  我们在外面定义一个数组，然后将这个数组传进去参数里面，我们在外面能知道这个数组名，并不是就一定是一个指针。因为在外面他是一个数组名，我们就能知道。这样就能sizeof得到这个数组的大小，然后就能知道数组有多少个元素。
  ...
  - 是会将格式中的字符都输入到字符串buffer中，这个函数就是拼写字符串的。format中的所有都会变成字符串，整数也会变成字符串，其他的格式也会在字符串中。
...
- 假设 p 是指向数组 arr 中第 n 个元素的指针，那么 *p++、*++p、(*p)++ 分别是什么意思呢？
  ...
  *++p 等价于 *(++p)，会先进行 ++p 运算，使得 p 的值增加，指向下一个元素，整体上相当于 *(p+1)，所以会获得第 n+1 个数组元素的值。
  ...
  (*p)++ 就非常简单了，会先取得第 n 个元素的值，再对该元素的值加 1。假设 p 指向第 0 个元素，并且第 0 个元素的值为 99，执行完该语句后，第 0 个元素的值就会变为 100。
      上面这种写法是对的，++本来就有左值，相当于元素本身的值+1，例如i++，相当于i = i + 1，并不需要左值，此时相当于元素本身+1，所以*p表示元素(*p)++表示     元素的值+1，++的写法不需要左值，前面的值本身就是左值。
  ```
...
- 除了字符数组，C语言还支持另外一种表示字符串的方法，就是直接使用一个指针指向字符串
  ...
  这两种都是字符串指针，字符数组在数组一节定义，上面的是数组指针不一样。
  ```
...
- 数组传递的时候最终会退化为指针。是将数组指针传递进来，并不是将整个数组传递进来。
...
- 所以在定义函数形参，传进来的是一个数组的时候可以有两种定义形式
  ...
  - 一种是定义一个数组的形式，一种是定义为指针的形式。最终都是退化为指针，结果都一样。
  - 上面的那种array可以当作一个数组指针使用，其对元素的访问包括数组名和指针变量在上面有解释，下面的这种就是一个指针，不能使用[]的数组指针的形式来访问元素。
    - 经过验证上面这句话是错误的，两种最后都是指针，都可以用[]的形式访问元素
...
- 二级指针可以指向一个具体的值，也可以指向一个指针数组，例如`int **`，当指向一个具体的值时代表指针指向一个`int *`，此时`int *`也是一个指针，指向一个具体的值。当指向一个指针数组时，指针指向一个`int *`，此时`int *`也是一个指针，如果此时用数组名作为指针的地址，数组指针指向的是数组中的一个具体元素，而不是整个数组，所以数组指针的类型和数组元素的类型有关，所以类型也是`int *`

- 二级指针，简单来说就是指向指针的指针。下面将从概念、用途、内存表示、示例代码等方面来详细解释如何理解二级指针。

  - 在 C 和 C++ 等编程语言中，指针是一种变量，它存储的是另一个变量的内存地址。而二级指针则是专门用来存储指针变量的地址。也就是说，普通指针指向的是数据，二级指针指向的是一个指针。

  - 假设我们有一个普通变量 `int num = 10;`，我们可以定义一个指针 `int *ptr = #` 来指向这个变量。此时，`ptr` 存储的是 `num` 的内存地址。如果我们再定义一个二级指针 `int **pptr = &ptr;`，那么 `pptr` 存储的就是 `ptr` 的内存地址。可以用下面的示意图来表示它们之间的关系：
  ...
  - 二级指针有多种实际用途，常见的有以下几个方面：

    - **动态内存分配管理**：在进行二维数组的动态内存分配时，二级指针能发挥很大作用。
    - **函数中修改指针的值**：当你需要在函数内部修改指针本身（而不是指针所指向的值）时，就需要传递指针的地址，也就是使用二级指针。
    - **多级数据结构操作**：在处理复杂的数据结构（如树、图等）时，可能会用到多级指针，二级指针是其中较为基础的一种。
...
- 根据上面的定义，我们可以对空指针做如下几点剖析：
  ...
  - 空指针表示“未分配”或者“尚未指向任何地方”。它与未初始化的指针有所不同，空指针可以确保不指向任何对象或函数，而未初始化指针可能指向任何地方。
    ...
    - 未初始化指针就是定义了但是没有初始化，此时指针可以指向任何地方，因为指针的地址是垃圾值，所以我们在定义指针的时候要初始化，初始化为空指针，这样我们就可以通过判断指针是否为空来进行后面的程序流程，如果没有初始化为空指针，指针是垃圾值，这样有值的话可能直接判断指针是否为空就会出现错误。所以一般都需要将指针初始化为空。

  - 0、0L、'\0'、3-3、0*17以及(void*)0等都是空指针常量，则：
    ...
    - 指针变量 p 经过上面任何一种赋值操作之后都将成为一个空指针。至于编译时系统究竟选取哪种形式作为空指针常量使用，则与具体实现相关。在一般情况下，对于 C 语言系统，选择“`（void*）0`”或 0 的居多（也有个别的选择 0L）；而对于 [C++](http://c.biancheng.net/cplus/) 语言系统，由于存在严格的类型转化的要求，“`void*`”不能像在 C 语言中那样自由转换为其他指针类型，所以通常只选 0 作为空指针常量，而不选择“`（void*）0`”。
...
- 在了解空指针的概念之后，下面来看 NULL 指针
  ...
  - 作为一种良好的编程习惯，很多程序员都不愿意在程序中到处出现未加修饰的 0 或者其他空指针常量。为了让程序中的空指针使用更加明确，从而保持统一的编程风格，标准 C 专门定义了一个标准预处理宏 NULL，其值为“空指针常量”，通常是 0 或者“((void*)0)”，即在指针上下文中的 NULL 与 0 是等价的，而未加修饰的 0 也是完全可以接受的。如在 VC++ 中定义预处理宏 NULL 的代码如下：
    ...
    - 这里需要说明的是，当 NULL 定义为“`((void *)0)`”时，即 NULL 是可以赋值给任何类型指针的值，它的类型为 void*，而不是整数 0，因此初始化“`FILE*fp=NULL`;”是完全合法的。0也完全可以。这里只是着重说了一下`(void *)0`
...
- 指针数组做为函数参数传入的时候，因为其是一个数组，所以数组名也会转化为一个指针，指针的类型为数组元素的类型即指针，所以需要两个**，一个代表数组元素的类型，一个代表此指针本身是一个指针。
  ...
  - 所以我们可以理解main函数中的argv`可以写为char * argv[] 或者 char ** argv`
    - 跟一维数组类似，可以写数组指针，也可以写数组定义的本身，`char ** argv`表示的是数组指针，`char * argv[]`表示这个指针数组定义的本身。

- 指针数组可以这样写，先定义几个一维数组，然后将一维数组的数组名放到指针数组里面，这样每个一维数组的大小就可以不一样了
...
- 指针数组里面每个元素代表一个指针即地址，但是通过这个地址不能判断指向的是一个值，还是一个数组。
  - 数组指针指向的是数组中的一个具体元素，而不是整个数组，所以数组指针的类型和数组元素的类型有关
  - 所以指针数组的元素指向一个数组，他的类型也是和数组元素的类型有关，指向一个具体的值不是数组，也是这个类型。所以不能区分。
  - 所以指针数组需要一个表示数组数量的值来确定数量，例如main函数中的argc代表指针数组的元素数量。所以在数组退化为指针的时候，我们需要有一个数量来表示数组的元素个数。
  - 所以指针数组里面的元素是一个指针，我们不知道是一个值还是一个数组，我们也需要一个表示元素个数的值来用，这样才不会越界。
...
- 使用指针 p 来访问二维数组中的每个元素:
  ...
  - `*(p+1)+1`表示第 1 行第 1 个元素的地址，如何理解？`*(p+1)`单独使用时表示的是第 1 行数据，放在表达式中会被转换为第 1 行数据的首地址，也就是第 1 行第 0 个元素的地址，因为使用整行数据没有实际的含义，编译器遇到这种情况都会转换为指向该行第 0 个元素的指针；就像一维数组的名字，在定义时或者和 sizeof、& 一起使用时才表示整个数组，出现在表达式中就会被转换为指向数组第 0 个元素的指针。
...
- 查看下面代码
  ...
  - 可能有人认为这段代码是正确的，因为[数组作为函数参数](https://so.csdn.net/so/search?q=数组作为函数参数&spm=1001.2101.3001.7020)时退化成一个指针，那么我对数组进行取址，这样&arr就是一个二维指针了，所以可以作为函数test的入参。
  ...
  - 这个错误是说，test函数期望的入参类型是`int **`。但是实际传入的参数类型是 `int (*)[4]`，即实际传入的类型是指向数组的指针
...
- 这种错误就是我之前在工作中犯的，从而导致出现异常的
  ...
  - 但是很遗憾，二维数组作为函数参数并不等价于二维指针。因为**数组作为函数参数时转换为指针没有传递性**。也就是说你不能认为一维数组和一维指针作为函数参数等价，就认为二维数组和二维指针就等价了。在C语言中没有这样的传递性。

  - 其实仔细想想，也是很容易明白的。二维数组其实就是一个数组的数组(即它是一个一维数组，只不过它的每个元素又都是一个一维数组)。当二维数组作为函数入参时，比如 `int a[3][4]`; 它的第一维数组会被转换成指针，相当于是传入了一个指向数组的指针。即作为函数参数， `int a[3][4]和 int (*p)[4]`等价。那 `int (*p)[4]和 int **pp`等价吗？肯定不等价， p指针指向类型是 `int [4]`，而pp指向的类型是`int *`。
...
- 指针数组存放的是指针，指针指向的可能是一个值，也可能是一个数组，都可以用指针数组。而二维数组指针每一行的元素个数是相同的，不可能不同。

- 指针数组基本上只用于字符串数组的情况，定义一些大小不同的字符串，然后将地址保存起来。

- 二维数组指针用在将二维数组作为参数传入函数的情况居多。因为二维数组会退化为二维数组指针，注意并不是退化为二级指针。

- 指针数组和二维数组指针在具体使用的时候在具体分析。

- 二级指针和二维数组指针的区别

  - 声明方式：
    - 二维数组指针：`int (*p)[4]`，指向一个二维数组的一行。
    - 二级指针：`int **p`，指向一个指针数组。
  - 内存布局：
    - 二维数组指针：内存是连续的，二维数组的元素在内存中是连续排列的。
    - 二级指针：内存不一定连续，每一行的内存是单独分配的。
  - 用法和访问方式：
    - 二维数组指针：通过`p[i][j]`访问元素
    - 二级指针：通过`p[i][j]`访问元素
...
- 除了可以对成员进行逐一赋值，也可以在定义时整体赋值struct stu{}stu1,stu2 = { "Tom", 12, 18, 'A', 136.5 };不过整体赋值仅限于定义结构体变量的时候，在使用过程中只能对成员逐一赋值，这和数组的赋值非常类似。定义结构体变量的时候初始化，并不是定义结构体结构的这个时候，是定义变量的时候，例如我们已经定义了student这个结构，然后我们在定义一个变量，struct student stu []这个是定义结构体变量。当然在定义结构体结构的时候也可以直接定义。
  ...
  - 上面这种初始化结构体数组的形式是正确的，上面说的只能对成员逐一赋值说的是先用ERROR_INFO定义一个结构体，但是没有定义，然后在后面使用的时候在用{}赋值，这是错误的，但是我们可以在定义结构的时候直接用{}初始化这个结构体变量

    - 我们要注意，数组和结构体使用{}初始化，这种操作是列表初始化，只能是初始化的时候用，后面在用的时候得是拷贝和赋值了，这种只能用其他变量来操作了，不能直接用初始化的操作。
    - 在没有定义的时候初始化，可以用memset统一初始化。
...
- 结构体里面定义指针，目前来看一般结构体里面只是定义字符串指针，例如上面定义了`char *name`，不管这个指针指向的数据有多大，这个结构的大小都是固定的，一个指针8个字节，在加上其余的字节数。字符串指向的数据是放在常量区的。所以我们在定义变量的时候可以直接写字符串指针指向的数据。因为用引号引起来的其实就是一个地址，只是在常量区。
...
- 一般情况下我们都是在定义的时候对它进行初始化，因为那样比较方便。如果定义之后再进行初始化，那就只能一个一个成员进行赋值，就同数组一样。
  ...
  - 这是在只有一个结构体的情况下，如果有两个结构体，其中一个结构体初始化之后，可以直接用这个结构体给另一个结构体赋值的。
    ...
    - 这种写法是对的，可以用其他的已经初始化的结构体赋值。

  - 上面说的定义之后在初始化说的是在只有一个结构体的情况下
    ...
    - 这种写法是错误的，在只有一个结构体变量的情况下，定义之后只能一个一个的赋值。
...
- 枚举定义时里面的大小可以相同，但是类型不可以相同，例如上面的定义中
  ...
  - 但是可以将所有的值都设置为1。这样定义是合法的，但是不怎么常用。

    ```
    enum week{ Mon = 1, Tues = 1, Wed = 1, Thurs = 1, Fri = 1, Sat = 1, Sun = 1 };
    ```
...
- 在C语言中，位域（bit field）只能在结构体（struct）或者共用体（union）中定义。位域允许你定义一个结构体或共用体的成员变量，并指定该成员变量占用的位数，从而实现更高效的内存利用。
...
- 按位与&，按位与运算通常用来对某些位清 0，或者保留某些位。
  ...
  - 取一个数X的某些位，该数的对应位为1，其余位为零，此数与X进行“与运算”可以得到X中的指定位。
    - 设X=10101110，取X的低4位，用 X & 0000 1111 = 00001110 即可得到

- 按位或|，按位或运算可以用来将某些位置 1，不能保留某些位，保留某些位用与运算
...
- 按位异或^，参与`^`运算两个二进制位不同时，结果为 1，相同时结果为 0，按位异或运算可以用来将某些二进制位反转。例如要把 n 的高 16 位反转，保留低 16 位，可以进行n ^ 0XFFFF0000运算
  - 异或计算时，如果与0进行异或，如果原来的位为1，结果为1，原来的位为0，结果为0，所以与0异或是保留原来的位
  - 如果与1进行异或，如果原来的位为1，结果为0，原来的位为0，结果为1，所以与1异或是将原来的位反转。
  
- 取反运算~，
- 左移运算<<，左移运算符`<<`用来把操作数的各个二进制位全部左移若干位，高位丢弃，低位补0。9<<3，表示9左移3位。如果数据较小，被丢弃的高位不包含 1，那么左移 n 位相当于乘以 2 的 n 次方。
  - 左移一位相当于原来的每一位转换为十进制时多乘以了2，例如二进制的111，转换为十进制时`1*2^2 + 1*2^1 + 1*2^0`，左移一位低位补0相当于1110，此时转换为十进制`1*2^3 + 1*2^2 + 1*2^1 + 0*2^0`,此时可以提出来一个2，0的位不参与运算，所以0相当于没有，相当于`2*二进制的111`，所以左移以为相当于乘以2
  - 综上，左移n位相当于乘以 2 的 n 次方
...
- 右移运算>>，右移运算符`>>`用来把操作数的各个二进制位全部右移若干位，低位丢弃，高位补 0 或 1。如果数据的最高位是 0，那么就补 0；如果最高位是 1，那么就补 1。如果被丢弃的低位不包含 1，那么右移 n 位相当于除以 2 的 n 次方（但被移除的位中经常会包含 1）。
  - 右移一位，如果丢弃的位是0，相当于每一位除以了2，0是不参与运算的，所以右移一位丢弃的是0相当于除以了2，丢弃的是1的话不能这样算
  - 综上，右移 n 位相当于除以 2 的 n 次方
...
- 为结构体定义别名
  ...
  - 在C语言中，使用 `typedef` 可以为结构体定义一个新的类型名，从而在声明变量时可以省略结构体的关键字。这种做法通常用于简化结构体类型的使用和增强代码的可读性。
    ...
    - 在上述示例中，`typedef` 关键字创建了一个新的类型名 `Point`，它代表了一个匿名结构体类型，其中包含两个整型成员 `x` 和 `y`。

    - 使用 `typedef` 后，可以直接使用 `Point` 来定义变量，而不必再重复写 `struct` 关键字和结构体名称

      ```c
      Point p1 = {10, 20};
      ```

  - 尽管可以使用 `typedef` 创建一个不带结构体名称的类型，但是在一些情况下，需要为结构体类型提供一个具体的名称，例如在定义结构体的时候：
    ...
    - 在上述示例中，`typedef` 同时为结构体定义了一个新类型名 `Node`，并且结构体自身也有了一个名称 `Node`，这样可以方便在结构体内部引用自身类型。
    - 经过验证，必须要这么写，否则直接给next变量用Node类型会出错，提示没有找到类型。
...
- 为枚举类型定义别名
  ...
  - 跟结构体一样，定义枚举类型别名时也可以不用具体得类型名称，可以直接定义别名，当然也可以写上具体得类型名称。
...
- const和指针：
  ...
  - 除此之外，也不能使用void`*`指针保存const对象的地址，必须使用const void`*`类型的指针保存const对象的地址。
    ...
    - 总体来说就是不能将const指针赋值给非const指针，p是const修饰的，所以`&p`取地址之后也是const的，所以上面是错误的。p是const int类型，表示p的值不能修改，所以`&p`取地址之后这个p值依然不能修改，所以取地址之后也是const类型的指针。
...
- const和非const类型转换：当一个指针变量 str1 被 const 限制时，并且类似`const char *str1`这种形式，说明指针指向的数据不能被修改；如果将 str1 赋值给另外一个未被 const 修饰的指针变量 str2，就有可能发生危险。因为通过 str1 不能修改数据，而赋值后通过 str2 能够修改数据了，意义发生了转变，所以编译器不提倡这种行为，会给出错误或警告。也就是说，`const char *`和`char *`是不同的类型，不能将`const char *`类型的数据赋值给`char *`类型的变量。但反过来是可以的，编译器允许将`char *`类型的数据赋值给`const char *`类型的变量。这种限制很容易理解，`char *`指向的数据有读取和写入权限，而`const char *`指向的数据只有读取权限，降低数据的权限不会带来任何问题，但提升数据的权限就有可能发生危险。C语言标准库中很多函数的参数都被 const 限制了，但我们在以前的编码过程中并没有注意这个问题，经常将非 const 类型的数据传递给 const 类型的形参，这样做从未引发任何副作用，原因就是上面讲到的，将非 const 类型转换为 const 类型是允许的。
  ...
  - 总结来说不能将const指针赋值给非const指针，但是其他的类型是可以的。例如

    ```
    const int constVar = 42;
    int nonConstVar;
    
    // 在C语言中是合法的，const 类型可以被赋值给非 const 类型
    nonConstVar = constVar;
    ```

    - nonConstVar就是一个int类型的变量，占有一个单独的地址，赋值就是将constVar变量地址的内容赋值给nonConstVar变量，两个地址不同，不能通过nonConstVar修改constVar中的值。
    - 但是指针赋值，虽然指针变量是两个，但是两个指针指的是一个地址，可以通过这个指针修改变量的内容，这样就错误了。


- 用const修改函数返回值，这样的函数返回的值是常量，不可被修改。主要用于返回值是一个指针的情况

  - 举一个返回值为const 指针的情况，并说明这样写的好处

    ```
    const int* func()
    ```

    - 防止通过函数返回值修改指针所指向的数据：当需要返回一个指针，但不希望[调用函数](https://so.csdn.net/so/search?q=调用函数&spm=1001.2101.3001.7020)的代码修改指针指向的数据时，可以使用常量整型指针。这可以避免意外的数据修改。
  ...
  - 通过上面const和非const类型转换可以看出，函数返回值为const指针，接收函数返回值的必须也得是一个const指针。

  - 如果函数返回值不是一个指针，只是一个普通的const，这就和普通的函数没什么区别，我们可以用一个const类型来接收返回值，也可以用非const类型来接收函数返回值。用const类型来接收更符合const函数的意思，但是也可以不用。知道这个区别就好
    ...
    int main() {
        const int result = getValue(); // 返回的值是常量
        // result = 10; // 错误，试图修改常量值
        std::cout << "Constant Value: " << result << std::endl;
        return 0;
    }
    ```

    - 上面这种写法更符合这个函数的意思，但是没有什么意思，只有返回值为const指针的时候才有用，返回值为非const指针的时候，我们完全可以赋值给一个非const类型的变量，然后修改新的变量的值就行了，因为这是两份数据，并不像指针一样指向一份数据

- c++中的引用相当于指针，所以返回值为const引用和指针一样，如果返回值为一个const引用，接收函数返回值的也必须得是一个const引用

- c语言中没有const_cast关键字来去除const属性，但是可以通过类型强转来去掉指针的const属性
  ...
  - 可以通过类型强转将const类型转换为非const类型，这种强转主要是针对于指针说的，其他普通类型不会这样，因为变量赋值会重新申请一块内存，而指针是指向同一块内存
  - 通过强转将`const int * a`转换为非const指针，然后通过非const指针来修改指向内存的值。
  
- const只是一种规范，但是我们可以绕过这个规范，但是一般情况下我们不这么弄

- 总的来看，c语言中的类型强转可以随意转换。
...
- extern用于函数
  ...
  - 对于函数来说在头文件和源文件中加不加extern含义一样，默认是可以省略的，但是我们也可以加上，但是如果在其他源文件中使用的时候，没有include对应的头文件，所以需要加extern 函数声明，来使用该函数。

##### 头文件注意事项

- c语言头文件一般定义各种函数，如果只用int char这种系统类型的话，是不用写include，也不用include stdio.h这些，相当于头文件中可以没有任何include，如果有使用其他定义类型如结构体，一般需要include，要不然找不到声明。
...
- 显式类型转换是通过在变量前面加上目标类型的括号来实现的。语法如下：
...
- \#define 叫做宏定义命令，它也是C语言预处理命令的一种。所谓宏定义，就是用一个标识符来表示一个字符串，如果在后面的代码中出现了该标识符，那么就全部替换成指定的字符串。
  ...
  ```c
  #define  宏名  字符串   //#表示这是一条预处理命令，所有的预处理命令都以 # 开头。宏名是标识符的一种，命名规则和变量相同。字符串可以是数字、表达式、if 语句、函数等。这里所说的字符串是一般意义上的字符序列
  ```
  ...
  - 宏定义必须写在函数之外，其作用域为宏定义命令起到源程序结束。如要终止其作用域可使用`#undef`命令。
    ...
    - #undef需要使用宏名，确认取消哪个宏的作用域
      ...
      int main() {
          ...
          // 重新定义PI
          #define PI 3.14159
          printf("Value of PI after redefining: %f\n", PI);
...
- 对于宏得错误理解
  ...
  - 原本认为宏只能是一个值，不能有很多值，我们自定义的只能有一个值，但是系统定义的例如上面这些，在展开时是编译器展开的，我们没有定义，编译器就按照系统理解展开了。
  - 宏是在预处理阶段展开得，当看到宏名时，展开成对应字符串，如果是我们自定义得，就按照我们定义得展开，如果是系统定义得，就按照系统设计展开，例如`__FILE__`，在预处理阶段，看到这个系统定义的宏名时，就展开成当前文件名，这是编译器干的事，因为编译器在编译源文件时知道文件名，直接就可以替换了，宏的作用就是展开，只是展开的方式不一样，有的是按我们自定义展开，有的是按系统定义展开，系统定义有自己的展开方式。
...
- #if用法的一般格式为：
  ...
  ```c
      ...
      // 若常量表达式1为真，编译此处代码
  #elif 常量表达式2
      // 若常量表达式1为假，常量表达式2为真，编译此处代码
  #else
      // 若上述常量表达式都为假，编译此处代码
  #endif  //#elif 和 #else 也可以省略
...
- #ifdef用法的一般格式--`#ifdef` 是 C 语言中的预处理指令，用于在编译时检查某个宏是否已经被定义。
...
- 在C语言中，`defined`是一个预处理运算符，用于在编译时检查某个宏是否被定义。它通常与条件预处理指令`#if`、`#elif`一起使用，以实现条件编译。
...
- 除了`defined`运算符，C语言还提供了`#ifdef`和`#ifndef`指令，用于简化检查宏是否被定义的过程：

  ```
  #ifdef` 等同于 `#if defined(MACRO)
  #ifndef` 等同于 `#if !defined(MACRO)
  ```
...
- 总体来说有两种方法来判断某个宏是否被定义，只是#ifdef只能判断一个，而#if可以写一个表达式
...
- 为什么用块和每个块的字节数
  ...
  - 为了灵活地处理不同大小的数据块，从而提高读取效率并满足多种数据读取需求。

  - `size`参数指定每个数据块的大小。对于不同的数据类型，这个大小会不同。例如：

    - 如果你读取的是字符数据，可以将`size`设为`sizeof(char)`，通常是1字节。
    - 如果你读取的是整数数据，可以将`size`设为`sizeof(int)`，可能是4字节或8字节，具体取决于系统。

  - `count`参数指定要读取的块的数量。例如：

    - 如果你读取的是单个整数数据块，可以将`count`设为1。
    - 如果你读取的是多个字符数据块，可以将`count`设为你想要读取的字符数量。

  - 假设有一个包含100个整数的二进制文件，你想读取其中的50个整数，可以这样做：
    ...
    int main() {
        ...
        result = fread(buffer, sizeof(int), 50, file);
        if (result != 50) {
    ...
    - 这样，`fread`函数会尝试读取50个整数数据块，每个数据块大小为4字节，共200字节的数据。如果成功，读取的数据将存储在`buffer`数组中。

  - 作用

    - **灵活性**：允许程序员指定要读取的任意大小和数量的数据块，适用于各种数据类型和文件格式。
    - **效率**：减少函数调用次数，提高读取大数据量时的效率。例如，一次读取100个字节比连续读取100次每次1个字节更高效。
    - **适应性**：便于处理结构化数据（如数组、结构体等），不需要每次读取一个字节并手动组合数据。
...
- read和fread以及write和fwrite区别

  - read就是通过read来实现的，[fread](https://so.csdn.net/so/search?q=fread&spm=1001.2101.3001.7020)是C语言的库，而read是系统调用。差别在read每次读的数据是调用者要求的大小，比如调用者要求读取10个字节数据，read就会从内核缓冲区（操作系统开辟的一段空间用来存储磁盘上的数据）读10个字节数据到[数组](https://so.csdn.net/so/search?q=数组&spm=1001.2101.3001.7020)中，所以每次调用read会涉及到用户态与內核态之间的切换从而损耗一定的性能。而fread不一样，为了加快读的速度，fread每次都会从内核缓冲区读比要求更多的数据，然后放到应用进程缓冲区(首地址存在FILE结构体中)，这样下次再读数据只需要到应用进程缓冲区中去取而无需过多的系统调用。
  - fwrite也是通过write来实现的，fwrite是C语言的库，而write是系统调用。差别在write每次写的数据是调用者要求的大小，比如调用者要求写入10个字节数据，write就会写10个字节数据到内核缓冲区中，所以依然涉及到用户态与內核态之间的切换，操作系统会定期地把这些存在内核缓冲区的数据写回磁盘中。而fwrite不一样，fwrite每次都会先把数据写入一个应用进程缓冲区，等到该缓冲区满了，或者调用类似调用fflush这种冲洗缓冲区的函数时，系统会调用write一次性把相应数据写进内核缓冲区中。同样减少了系统调用(即write调用)。
  ...
  - 如果程序对内存有限制，则用read/write比较好。fread和fwrite，它会自动分配缓存，速度会很快，比自己来做就要简单。如果要处理一些特殊的文件描述符，用read和write,如套接口，管道之类的
  - read和write也会设计到缓冲区，只不过这个缓冲区是内核缓冲区，而fread和fwrite增加了应用程序缓冲区，更少的调用系统函数，减少了用户态和内核态的切换。
...
- 结构体内存对齐原则
  ...
  - 每个成员的首地址是自身大小的整数倍
    ...
    - 此处说的是一个结构体成员的首地址是其自身大小的整数倍，目前这么记住就可以了，不要纠结为什么
      ...
      - 经过验证，a占用的地址是第0个，然后b变量占用的地址为第8个开始的，因为double大小为8，所以变量的首地址为自身大小的整数倍，只能是从8开始。

        - 64位系统cpu一次读8个字节，如果从第4个字节开始，这就需要读两次，所以需要按照自身大小的整数倍来字节对齐
      ...
      - 这样考虑一下，如果有一个结构体
        ...
        - 经过验证，a是从0开始的，b是从4开始的。但是为什么64位系统一次读取8个字节，为什么b不能从1开始，这样也不用读两次，读一次分别赋值就行了，我猜测，主要是为了计算机好计算以及好按字节大小给分开，要不然还得计算从哪开始哪几个字节是哪个变量的。总体来说要记住每个成员的首地址是自身大小的整数倍。

      - 上面说的都是基本数据结构，如果一个结构体中包含另一个结构体，被包含的结构体的第一个变量的首地址为被包含结构体中变量所占字节数最大的那一个的整数倍，相当于被包含结构体的自身大小就是被包含结构体中所有变量中占字节数最大的那一个。
        ...
        - 上面一个结构体包含一个结构体，经过验证，c的地址为0，a的地址为8，因为被包含结构体占用字节数最大的变量是8，所以被包含结构体的首地址为8的整数倍即8，如果b的类型不是double而是int，此时a的首地址为4

  - 结构体的总大小，为其成员中所含最大类型的整数倍。

    - 这个最大类型指的是基本数据类型，并不是指如果一个结构体里面包含另一个结构体，结构体的总大小为被包含结构体类型的整数倍，因为被包含的结构体也是基本类型组成的，所以记住最大类型指的是基本数据类型就可以了，不能将被包含结构体整个的大小作为一个最大类型。

  - 结构体变量的首地址能够被其对齐字节数大小所整除

- 如何验证c语言里面的变量地址

  - `%p`可以打印地址，重要的是地址，打印的时候我们需要用&来取地址然后打印，不能直接打印变量。

    ```
    printf("%p\n", &ABC.c);
    这样取变量地址然后打印就可以查看变量在内存中的地址
    printf("%p\n", ABC.c);
    这样不取地址是错误的，打印出来的很乱，但是结构体中的变量在内存中是挨着存放的。
    ```
...
- <img src="http://c.biancheng.net/uploads/allimg/190122/104P51I1-0.jpg" alt="img" style="zoom: 80%;" />
...
- 内存中常量区存储的都是什么数据
  ...
  - 在 C、C++ 等编程语言里，内存中的常量区是用来存放常量数据的一块特殊内存区域。常量区又可细分为只读数据段（`.rodata`）和字符串常量区，以下是对常量区所存储数据的详细介绍：

  - 字符串常量--字符串常量是用双引号括起来的字符序列，像 `"Hello, World!"` 这样的。在程序运行期间，这些字符串常量会被存于常量区，并且其内容是不能被修改的。
    ...
    - 在这个示例中，`"Hello, World!"` 就是一个字符串常量，它会被存于常量区，`str` 指针指向该字符串常量的首地址。

  - 全局常量和静态常量--全局常量和静态常量在定义时就被赋予了固定的值，并且在程序的整个生命周期内都不会改变。这些常量同样会被存于常量区。
  ...
  - 枚举常量--枚举类型定义的常量也会被存于常量区。枚举常量代表了一组命名的整数值。
    ...
    - 在这个示例中，`MONDAY`、`TUESDAY` 等枚举常量会被存于常量区。

  - 编译器优化产生的常量--编译器在编译过程中可能会对一些表达式进行优化，把计算结果作为常量存于常量区。

    ```c
    #include <stdio.h>
    
    int main() {
        // 编译器优化后，1 + 2 会被计算为 3 存储在常量区
    ...
    - 在这个示例中，`1 + 2` 的计算结果 `3` 会被当作常量存于常量区。

- 上述中的hello world和3都是有变量来接受，如果没有变量接收，比如我直接写一个参数hello world写入调用的函数中，这个hello world是在常量区吗

  - 是的，即便没有变量来接收，直接将 `"Hello, World!"` 这样的字符串字面量作为参数传递给函数，它依然存储在常量区。下面为你详细解释：
...
- 那如果上面的3也是直接调用函数传进去呢，也是放在常量区吗

  - 在 C 语言里，如果直接把像 `3` 这样的常量作为参数传递给函数，它并不会像字符串字面量那样存储在常量区。下面来详细分析：

  - 当你把整数常量（如 `3`）作为参数传递给函数时，它的处理方式和字符串字面量有所不同。在函数调用过程中，这个常量值会被直接嵌入到指令流里，或者被存储在寄存器中。
    ...
    - **编译阶段**：编译器在处理 `printNumber(3)` 时，会把 `3` 作为立即数嵌入到函数调用的指令中。比如在 x86 架构下，函数调用时参数可能会通过栈或者寄存器传递。对于像 `3` 这样简单的常量，它可能会被直接编码到汇编指令里，用于初始化相应的寄存器或者压入栈中。
    - **运行阶段**：程序执行到 `printNumber(3)` 时，CPU 会根据指令中的立即数 `3` 对相关寄存器或者栈进行操作，然后跳转到 `printNumber` 函数执行。整个过程中，`3` 并没有像字符串字面量那样被存储在专门的常量区，而是直接参与指令的执行。

- 上述两者的区别

  - 字符串字面量 `"Hello, World!"` 因为是多个字符组成的序列，需要在内存中占据连续的空间，并且在程序运行期间可能会被多个地方引用，所以被存储在常量区。而像 `3` 这样的简单整数常量，由于其值较小且易于在指令中直接表示，就不需要专门存储在常量区了。
...
- 要想规避野指针，就要养成良好的编程习惯：
  ...
  - 当指针指向的内存被释放掉时，要将指针的值设置为 NULL，因为 free() 只是释放掉了内存，并未改变指针的值。
...
- 头文件防止重复包含，宏保护
  ...
  - 应该是如果定义了宏，编译起在看见宏已经定义之后，就不展开这个头文件在源文件中了
...
- 静态全局变量
  ...
  - 这两者的区别虽在于：
    ...
    - 上面这句话是正确的，一般情况下不在头文件中定义static变量
    - 在C语言中，如果在一个头文件中定义了一个`static`变量，那么在其他源文件中包含此头文件之后，无法直接使用这个`static`变量。这是因为`static`变量的作用域限制在定义它的编译单元内，每个包含该头文件的源文件都会有自己独立的、不可见的`static`变量实例。
    - 相当于每一个源文件中都有一个static变量，跟static变量的使用不一样了。
...
- 锁的作用就是我们对一个变量进行约定，如果拿到那把锁，我们就对变量进行操作，如果没有拿到锁，就等待，所以我们可以设置好几把锁，每把锁商议的内容不一样，例如第一把锁控制变量A，第二把所控制变量B，如果我们想操作变量A，就先拿到第一把锁，这样才能操作，这是一种约定，程序员自己设置的约定。不一定全部的变量就是一把锁。
...
- 读写锁适用于读多写少的场景，而且能明确区分读操作和写操作的场景，即多个线程中读线程多，写线程少，而且多个线程只会读不会写，这样就能区分出来，就能使用读写锁。如果一些线程即能读又能写一些锁上的变量，这时就应该用互斥锁，这样又读又写的时候就阻止其他线程访问。即为理解为如果定义一个读写锁，我们不能在一个线程中即获取读锁又获取写锁，这样就会错误，不如用读写锁。即读写锁中一个线程只能获取一种锁，不是读锁就是写锁。
...
- windows下使用send和recv函数来向套接字读写数据，linux下也同样有这两个函数来实现套接字通信功能，这样保持不同平台一样。
  ...
  - recv、send是单独为[Socket](https://so.csdn.net/so/search?q=Socket&spm=1001.2101.3001.7020)套接字设计的接口，只能用于套接字通信；
  - read、write是操作系统调用，是对文件读写设计的接口，只要是对文件进行操作，都可以使用此接口， 比如[套接字](https://so.csdn.net/so/search?q=套接字&spm=1001.2101.3001.7020)操作（Linux中一切都是文件，包括套接字）；
...
- 第四个参数可以是0或者是下面的组合
  ...
  - MSG_DONTROUTE:是**send**函数使用的标志.这个标志告诉IP.目的主机在本地网络上面,没有必要查找表.这个标志一般用网络诊断和路由程序里面.
  ...
  - MSG_OOB:表示可以接收和发送带外的数据
    ...
    - 我们先来介绍TCP发送带外数据的过程。假设一个进程已经往某个TCP连接的发送缓冲区中写入了N字节的普通数据，并等待其发送。在数据被发送前，该进程又向这个连接写入了3字节的带外数据“abc”。此时，待发送的TCP报文段的头部将被设置URG标志，并且紧急指针被设置为指向最后一个带外数据的下一字节（进一步减去当前TCP报文段的序号值得到其头部中的紧急偏移值）

      ![](https://yqfile.alicdn.com/4f13cff352bb51e3d883bccee82afe7d15cafe0c.png)

    - 由图可见，发送端一次发送的多字节的带外数据中只有最后一字节被当作带外数据（字母c），而其他数据（字母a和b）被当成了普通数据。如果TCP模块以多个TCP报文段来发送图所示TCP发送缓冲区中的内容，则每个TCP报文段都将设置URG标志，并且它们的紧急指针指向同一个位置（数据流中带外数据的下一个位置），但只有一个TCP报文段真正携带带外数据。
    ...
    - 带外数据的缓冲区只有一个字节，所以发送多个字节的时候只有最后一个字节被认为是带外数据。

      ```
      Send(sockfd, "4", 1, MSG_OOB);
      Recv(connfd, buff, sizeof(buff)-1, MSG_OOB);
      ```

      - 虽然读取到buffer中，但是这个只会读到一个。
