...
- 这些地址用于局域网（LAN）内，不会在互联网中路由，因此它们可以在不同的局域网中重复使用。
- **A 类私有地址**：`10.0.0.0` 到 `10.255.255.255`（子网掩码 `255.0.0.0`）
- **B 类私有地址**：`172.16.0.0` 到 `172.31.255.255`（子网掩码 `255.240.0.0`）
- **C 类私有地址**：`192.168.0.0` 到 `192.168.255.255`（子网掩码 `255.255.0.0`）
