- shell变量
   - Bash shell默认情况下不会区分变量类型，即使将蒸熟和小树赋值给变量，也会被视为字符串，可以使用declare关键字显示定义变量的类型，一般情况下没有这个需求
...
- shell命令替换
   ...
   - 可以使用；使用多个命令例如$(dirname $0;pwd)，dirname命令去除文件名中的非目录部分，仅仅显示与目录有关的内容，dirname读取指定路径名最后一个/，提高程序的可移植性，就是没有了/home/lyl这些前面的东西，直接跳到父目录里面了。
...
- shell字符串
   ...
   - shell字符串截取
      ...
      - 从指定字符开始截取 ${string#\*chars}使用#可以截取指定字符(或子字符串)右边的所有字符，*表示忽略左边的所有字符直到指定chars，如果不需要忽略chars左边的字符，可以不写\*，但是chars需要从头写起，上述写法是遇到第一个匹配的字符就结束了，如果希望直到最后一个指定字符，可以使用##,${string##\*chars
```shell
str="---aa+++aa@@@"
echo ${str#*aa}   #结果为 +++aa@@@
echo ${str##*aa}  #结果为 @@@
```
      - 使用%截取指定字符或字符串左边的所有字符，因为要截取chars左边的字符，忽略chars右边的字符，所以\*应该位于chars的右侧，其他方面%和#用法相同，%是从右往左查找。${string%chars\*}
- shell数组
