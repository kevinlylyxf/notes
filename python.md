...
- GNU Compiler Collection 包括 C、C++、Objective-C、Fortran、Ada、Go 和 D 的前端，以及这些语言的库（libstdc++，...）。
- GCC大写的是GNU Compiler Collection的首字符，表示GNU编译器套件，里面包含各种编译器。例如编译器gcc是编译c语言的，gccgo是编译go语言的，g++是编译c++的。GCC只是一个统称，我们需要编译什么语言就需要下载对应的编译器。
- 并不是所有的编译型语言都会经历四个过程预处理、编译、汇编、链接，这四个过程是c和c++要经历的，这是其编译器决定的，我们不要想当然认为其他的也是要经历这四个过程，例如go语言，Go 的编译器在逻辑上可以被分成四个阶段：词法与语法分析、类型检查和AST 转换、通用SSA 生成和最后的机器代码生成，这个就不能按照上面的四个过程来算，而且不能使用gcc -E -c -o这些选项来处理了，每个编译器都有不同的命令，不要都按gcc的来处理和记忆。ada的编译器会生成ali文件，链接的是ali文件，所以都不一样，不要都按gcc记忆，gcc和GCC是不一样的，GCC是套件，gcc是具体的c语言编译器。
- 这些编译型语言最后都会生成ELF文件，所有的都会链接动态库，这些动态库里面会包含一些基本的数据处理和文件处理这些函数，如果是自己写的函数，c和c++可以编译为动态库，也可以编译为目标文件由链接器链接完使用，其他的语言也应该是这样的，但是其他的语言自己写的函数应该不会编译为动态库来使用，应该是类似于目标文件这样，但是具体的是什么情况应该是按照自己的来处理的，并不是都要成为目标文件，而且有的没有目标文件。
- 总体理解来说，只要最后能变成机器码就可以了，不一定非要是目标文件这种方式。只要是机器码。
