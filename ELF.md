...
<img src="/Users/lyl/Documents/notes/pictures/linux-kernel/2.png" alt="2" style="zoom: 50%;" />
...
装载时重定位是解决动态模块中有绝对地址引用的方法之一，但是它有一个很大的缺点是指令部分无法在多个进程之间共享。

我们希望程序模块中共享的指令部分在装载时不需要因为装载地址的改变而改变，所以实现的基本思想就是把指令中那些需要被修改的部分分离出来，跟数据部分放在一起，这样指令部分就可以保持不变，而数据部分可以在每个进程中拥有一个副本，这种方案就是地址无关代码。
...
<img src="/Users/lyl/Documents/notes/pictures/linux-kernel/6.png" alt="6" style="zoom: 50%;" />

#### 模块外部的数据访问

对于模块间的数据访问，ELF会在数据段里面建立一个指向这些变量的指针数组，称为全局偏移表（Global Offset Table，GOT）。 基本机制如下图所示：

<img src="/Users/lyl/Documents/notes/pictures/linux-kernel/7.png" alt="7" style="zoom: 33%;" />
...
<img src="/Users/lyl/Documents/notes/pictures/linux-kernel/8.png" alt="8" style="zoom: 33%;" />
...
表的索引。GOT：全局偏移表（GOT）的地址。

L：某个符号的过程链接表项的位置（节区偏移/地址）。过程链接表项

把函数调用重定位到正确的目标位置。链接编辑器构造初始的过程链

接表，动态链接器在执行过程中修改这些项目。
...
重定位类型可以用readelf -r elf文件名查看；

用代码读取重定位类型的具体方式如下：

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsaF2MDS.jpg) 

先读取到r_info，然后用它和”0xff”与（64位的话，是和“0xffffffff”与，即可得出重定位的类型；数值与类型的对应关系见下面的截图：

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsKTVdbX.jpg) 

 

链接器部分重定位的代码在“glibc-2.12.2/sysdeps/i386/dl-machine.h”中（这是i386处理器的，其他处理器的在别的目录）的elf_machine_rel函数（第312行）和elf_machine_rela函数（第491行）。

在glibc-2.12.2/elf/do-rel.h文件的elf_dynamic_do_rel函数中对elf_machine_rel进行了调用（如果是rela类型的，则调用elf_machine_rel）

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpseTV87n.jpg) 

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsce3asL.jpg) 

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsySNZ8A.jpg) 

重定位的类型是与处理器相关的，下面来详细介绍x86（32位为例）平台的10种重定位

#### R_386_32

  ![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsOAi91y.jpg)
  R_386_32计算公式：S+A。
  S：对于.o文件，表示全局变量被链接器安排的位置或其所在xx节的起始位置；对于.so文件，表示全局变量被加载到的虚拟地址。
  A：被重定位处的原始值（4字节，由重定位项offset指向）。

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsQm9olP.jpg) 

#### R_386_PC32

  ![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsbIxvq9.jpg)
...
![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsQ5BB1f.jpg) 

#### R_386_GOT32

  ![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wps93s8Vu.jpg)
...
#### R_386_GLOB_DAT
  ...
  ![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsQB0EEU.jpg)
  R_386_GLOB_DAT计算公式：S。

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsc2ZrHD.jpg) 

#### R_386_PLT32

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsm7H1DR.jpg)
...
![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsQ0fdfJ.jpg)
  R_386_JUMP_SLOT计算公式：S

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsfAn1fW.jpg) 

#### R_386_COPY

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsv6jl22.jpg)
  R_386_COPY计算公式：无

链接编辑器创建这种重定位类型的目的是支持动态链接。其偏移量成员引用某个可写段中的某个位置。符号表索引规定符号应该既存在于当前目标文件中，也存在于某个共享目标中。在执行过程中，动态链接器把与共享目标的符号相关的数据复制到由偏移给出的位置。

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsqTUU2b.jpg) 

#### R_386_RELATIVE

 ![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsMdQrxW.jpg)
  R_386_RELATIVE计算公式：B+A
  B：.so文件加载地址。

 

![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsUjvrbg.jpg) 

#### R_386_GOTOFF

  ![img](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsnegXLm.jpg)
...
![](file:////private/var/folders/xb/s2j9v4kj6y36zhhz0ywj2ph80000gn/T/com.kingsoft.wpsoffice.mac/wps-lyl/ksohtml/wpsWZQhAf.jpg)
